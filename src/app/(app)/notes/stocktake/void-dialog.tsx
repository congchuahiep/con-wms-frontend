...
import { type StocktakeNote, useVoidStocktakeNote } from "@/features/stocktake";

interface VoidStocktakeNoteDialogProps {
  note: StocktakeNote | null;
  onClose: () => void;
}

export function VoidStocktakeNoteDialog({
  note,
  onClose,
}: VoidStocktakeNoteDialogProps) {
        ...
        {note && (
          <Form of={form} onSubmit={handleSubmit} className="space-y-4">
            ...
            <TextareaField
              ...
              path={["reason"]}
              label="Lý do hủy"
              placeholder="VD: Kiểm kê nhầm kho"
              required
            />

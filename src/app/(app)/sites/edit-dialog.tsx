...
import { type Site, toSiteInput, useUpdateSite } from "@/features/site";
  ...
  return (
    ...
    >
      <DialogContent className="w-lg">
        {site && <EditSiteForm site={site} onClose={() => setOpen(false)} />}
      </DialogContent>
    </Dialog>
  );
}
...
function EditSiteForm({ site, onClose }: EditSiteFormProps) {
  const { form, handleSubmit, isPending, isDirty, error } = useUpdateSite(
    site.id,
    toSiteInput(site),
    {
      onSuccess: () => {
        toast.add({
          title: "Thành công",
          description: "Đã cập nhật công trường",
        });
        onClose();
      },
    },
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>Sửa công trường</DialogTitle>
        <DialogDescription>
          Chỉnh sửa thông tin công trường &quot;{site.name}&quot;
        </DialogDescription>
      </DialogHeader>
